...
const ChessScreen = ({ navigation }: any) => {
  ...
  const handleNewChallenge = (data: any) => {
    if (data.to === user?._id) {
      ...
      showToast('Chess Challenge', `${data.fromName} challenged you!`, 'info');
    }
  };
  ...
  const handleAcceptChallenge = (challenge: ChessChallenge) => {
    ...
    const roomId = `chess_${challenge.challenger._id}_${user?._id}_${Date.now()}`;
  ...
  const handleDeclineChallenge = (challenge: ChessChallenge) => {
    if (!socket) {
      showToast('Error', 'Not connected to server', 'error');
      return;
    }
  ...
  const renderChallenge = ({ item }: { item: ChessChallenge }) => {
